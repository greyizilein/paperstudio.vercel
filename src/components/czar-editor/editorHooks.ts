...
export function useCzDictation(): DictationState {
  ...
  useEffect(() => {
    if (!live) return;
    setStep(0); setSeconds(0); setPhase('interim');
  ...
  const pair = CZ_FAKE_SCRIPT[Math.min(step, CZ_FAKE_SCRIPT.length - 1)];
  const final = phase === 'final' ? pair[1] : pair[0];
  const interim = phase === 'interim' ? pair[1].slice(pair[0].length) : '';

  return { live, seconds, final, interim, start: () => setLive(true), stop: () => setLive(false) };
}
...
export function useCzDropZone(): DropZoneState {
  ...
  const onDrop = (e: React.DragEvent) => {
    ...
    if (f) {
      ...
      setImported({ name: f.name, kind: ext.toUpperCase(), size: (f.size / 1024).toFixed(1) + ' KB', words: words.toLocaleString() });
