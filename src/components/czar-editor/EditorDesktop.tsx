...
function CzTitlebar({ docTitle, setDocTitle, mode, setMode, onOpenSettings }: TitlebarProps) {
  return (
      ...
      <div className="cz-titlebar-actions">
        <div className="cz-mode-tabs">
          {["Edit", "Read", "Voice"].map((m) => (
            <button key={m} data-active={mode === m.toLowerCase() ? "true" : "false"}
...
function CzComposer({ format, setFormat, dictLive, onMicToggle, onOpenSettings }: ComposerProps) {
  const tog = (k: keyof FormatState) => setFormat({ ...format, [k]: !format[k as "b" | "i" | "u" | "s"] });
    ...
    <div className="cz-composer">
      <CzCGroup label="Style">
        <select className="cz-csel cz-csel-wide" value={format.block} onChange={(e) => set("block", e.target.value)}>
          ...
          <option>Pull quote</option>
          <option>Lede paragraph</option>
        </select>
      </CzCGroup>
      ...
      <CzCGroup label="Format">
        <button className="cz-cbtn cz-cbtn-bold"   data-active={format.b ? "true" : "false"} onClick={() => tog("b")}>B</button>
        <button className="cz-cbtn cz-cbtn-italic" data-active={format.i ? "true" : "false"} onClick={() => tog("i")}>I</button>
        <button className="cz-cbtn cz-cbtn-under"  data-active={format.u ? "true" : "false"} onClick={() => tog("u")}>U</button>
        <button className="cz-cbtn cz-cbtn-strike" data-active={format.s ? "true" : "false"} onClick={() => tog("s")}>S</button>
      ...
      <CzCGroup label="Color">
        <button className="cz-cbtn cz-cbtn-color" title="Text color" style={{ "--swatch": "var(--primary)" } as React.CSSProperties}>A</button>
        <button className="cz-cbtn cz-cbtn-color" title="Highlight" style={{ "--swatch": "hsl(50 95% 70%)" } as React.CSSProperties}>
          <span style={{ background: "hsl(50 95% 70% / 0.4)", padding: "0 3px", borderRadius: 2 }}>a</span>
        </button>
      </CzCGroup>

      <CzCGroup label="Align">
        <button className="cz-cbtn" data-active={format.align === "left" ? "true" : "false"}    onClick={() => set("align", "left")}    title="Left">⫷</button>
        <button className="cz-cbtn" data-active={format.align === "center" ? "true" : "false"}  onClick={() => set("align", "center")}  title="Center">⫶</button>
        <button className="cz-cbtn" data-active={format.align === "right" ? "true" : "false"}   onClick={() => set("align", "right")}   title="Right">⫸</button>
        <button className="cz-cbtn" data-active={format.align === "justify" ? "true" : "false"} onClick={() => set("align", "justify")} title="Justify">≡</button>
      </CzCGroup>
      ...
      <CzCGroup label="Czar AI" ai>
        <button className="cz-cbtn" title="Tighten — cut filler"><i>§</i> Tighten</button>
        <button className="cz-cbtn" title="Shift the voice" onClick={() => onOpenSettings("modes")}><i>§</i> Voice…</button>
        <button className="cz-cbtn" title="Continue from here"><i>§</i> Continue</button>
      </CzCGroup>

      <CzCGroup label="Dictate">
        <button className="cz-cbtn" data-live={dictLive ? "true" : "false"} title="Dictate (⌥ Space)"
          onClick={onMicToggle} style={{ minWidth: 30, fontSize: 14 }}>
          {dictLive ? "◉" : "◎"}
        </button>
        <button className="cz-cbtn" title="Dictation settings" onClick={() => onOpenSettings("dictation")} style={{ fontSize: 11 }}>EN-US</button>
      </CzCGroup>
  ...
  return (
    <aside className="cz-rail">
      <div className="cz-rail-section">
        ...
        {CZ_PIECES.map((p) => (
          <div key={p.id} className="cz-piece" data-active={p.active ? "true" : "false"}>
      ...
      <div className="cz-rail-section">
        ...
        {CZ_OUTLINE.map((o) => (
          <div key={o.id} className="cz-outline-item"
            data-level={o.level} data-current={o.current ? "true" : "false"}>{o.text}</div>
        ))}
      </div>
...
interface LeafProps {
  dragOver: boolean;
  imported: import("./editorHooks").ImportedFile | null;
  onImportClose: () => void;
  handlers: Record<string, React.EventHandler<React.DragEvent>>;
}
function CzLeaf({ dragOver, imported, onImportClose, handlers }: LeafProps) {
  return (
    <main className="cz-canvas" {...handlers}>
      <CzDropOverlay visible={dragOver} />
      <CzImportedChip file={imported} onClose={onImportClose} />
      <article className="cz-leaf">
        ...
        <p>
          ...
          I want to apologise for — we broke{" "}
          <span className="cz-flag cz-flag-grammar">on a Wednesday, of course</span>,
...
interface InspectorProps {
  ...
  suggestions: string[];
  setSuggestions: (ids: string[]) => void;
}
function CzInspector({ voice, length, setLength, audience, setAudience, suggestions, setSuggestions }: InspectorProps) {
  const v = CZ_VOICES.find((x) => x.id === voice) ?? CZ_VOICES[0];
  const dismiss = (id: string) => setSuggestions(suggestions.filter((s) => s !== id));
  const activeSuggs = CZ_SUGGESTIONS.filter((s) => !suggestions.includes(s.id));
  ...
  return (
    <aside className="cz-inspector">
        ...
        <div className="cz-insp-row">
          ...
          <div className="cz-length-pill">
            {["short", "medium", "long", "epic"].map((l) => (
              <button key={l} data-active={length === l ? "true" : "false"} onClick={() => setLength(l)}>{l}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="cz-insp-section">
        <div className="cz-insp-h">
          Czar is reading
          <button className="cz-insp-h-action">{activeSuggs.length}</button>
        </div>
        {activeSuggs.map((s) => (
...
export function CzarDesktop() {
  ...
  const [settingsSection, setSettingsSection] = useState("modes");
  ...
  const openSettings = (section?: string) => {
    setSettingsSection(section ?? "modes");
    setSettingsOpen(true);
  };

  return (
    <div className="cz-app" data-mode={lightMode ? "light" : "dark"}>
      ...
      <div className="cz-stage">
        ...
        <CzLeaf
          ...
          imported={drop.imported}
          onImportClose={() => drop.setImported(null)}
          handlers={drop.handlers}
        />
        <CzInspector
          voice={voice} length={length} setLength={setLength}
          audience={audience} setAudience={setAudience}
          suggestions={dismissedSuggestions} setSuggestions={setDismissedSuggestions}
        />
      </div>
      <CzStatus />
      <CzMicPopover
        open={dict.live} dict={dict}
        onClose={() => dict.stop()}
      ...
      />
      {/* Light mode toggle hidden in status bar — wire via preferences */}
    </div>
  );
}
