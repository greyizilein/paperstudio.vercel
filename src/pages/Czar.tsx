...
import {
  ...
  type CzarConversation, type CzarMessage,
} from "@/lib/czarStream";
import { ConvSidebar } from "@/components/czar/ConvSidebar";
import { UpgradeModal } from "@/components/czar/UpgradeModal";
import { DashboardSidebar } from "@/components/dashboard/DashboardSidebar";

// ── lazy-load heavy components so the page shell paints instantly ──
import { lazy, Suspense } from "react";
const VoiceInput = lazy(() => import("@/components/czar/VoiceInput").then(m => ({ default: m.VoiceInput })));
...
export default function CzarPage() {
  ...
  const loadConv = useCallback(async (id: string) => {
    try {
      ...
      // Rebuild document content from assistant messages
      ...
      // silently ignore load errors — user will see empty conversation
    }
  }, []);
  ...
  const sendMessage = useCallback(async (text: string, files: File[]) => {
    ...
    // Switch to document tab on mobile when writing
    ...
    try {
      ...
      const handlers: CzarHandlers = {
        ...
        onAgent: (e: CzarAgentEvent) => {
          setAgents(prev => {
            const existing = prev.findIndex(a => a.id === e.id);
            if (existing >= 0) {
              const next = [...prev];
              next[existing] = { ...next[existing], ...e };
              return next;
            }
        ...
        onDelta: (text: string) => {
          ...
          // Mirror to document panel for write/correct/research modes
          if (["write", "correct", "research"].includes(mode)) {
            setDocContent(accText);
          }
        },
        onTool: (_e: CzarToolEvent) => {
          // Tool calls are shown via agent dock events
        },
        onStatus: (e) => {
  ...
  const handleSelectionAction = useCallback((action: string, selectedText: string) => {
    if (action === "export") {
      // Trigger docx download — placeholder
      const blob = new Blob([docContent], { type: "text/plain" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = "czar-document.txt";
      a.click();
  ...
  return (
    <div className="flex h-[100dvh] bg-background overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-0">
        ...
        {/* Top bar */}
        <header className="flex items-center gap-2 px-4 h-12 border-b border-border flex-shrink-0 bg-background/95 backdrop-blur">
          {/* Sidebar toggle */}
          <button
          ...
          <div className="relative">
            <button
              ...
              disabled={streaming}
              className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[12px] font-semibold transition-colors disabled:opacity-50 ${MODE_COLOURS[mode]}`}
            >
              {modeIcon(mode)}
              {modeLabel(mode)}
              <ChevronDown size={11} />
            ...
            {showModeMenu && (
              <>
                ...
                <div className="absolute top-full mt-1 left-0 z-20 bg-background border border-border rounded-xl shadow-xl overflow-hidden w-44 animate-in fade-in zoom-in-95 duration-150">
                  {(["chat", "write", "correct", "research", "plan"] as CzarMode[]).map(m => (
                    <button
                      key={m}
                      onClick={() => { setMode(m); setShowModeMenu(false); }}
                      className={`w-full flex items-center gap-2 px-3 py-2 text-[12px] font-medium hover:bg-secondary transition-colors text-left ${mode === m ? "text-foreground font-semibold" : "text-muted-foreground"}`}
                    >
                      {modeIcon(m)}
                      {modeLabel(m)}
                      {mode === m && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-primary" />}
                    </button>
                  ))}
                </div>
              </>
          ...
          <div className="ml-auto flex items-center gap-1.5">
            ...
            {docContent && !streaming && (
              <button
                onClick={() => handleSelectionAction("export", "")}
        ...
        <div className="flex-1 min-h-0 flex">
          ...
          <div className={`flex flex-col min-h-0 ${
            isDocMode
              ? "w-full lg:w-[420px] lg:border-r lg:border-border"
              : "w-full"
            ...
            <div className="lg:hidden flex-1 overflow-y-auto p-4">
              ...
              {agents.length === 0 && (
                <div className="flex flex-col items-center justify-center h-48 text-center">
                  ...
                  <p className="text-[11px] text-muted-foreground/60 mt-1">Agents appear here when CZAR is working on a task</p>
                </div>
              )}
            </div>
          )}
    ...
    <div className={`flex gap-3 ${isUser ? "justify-end" : "justify-start"}`}>
      ...
      <div className={`max-w-[85%] lg:max-w-[75%] ${isUser ? "order-first" : ""}`}>
        ...
        {msg.mode && msg.mode !== "chat" && !isUser && (
          <span className="text-[10px] text-muted-foreground/50 mt-1 block ml-1">{msg.mode} mode</span>
        )}
      </div>
      {isUser && (
...
function WelcomeScreen({ mode, onExample }: { mode: CzarMode; onExample: (t: string) => void }) {
  ...
  const icons: Record<CzarMode, React.ReactNode> = {
    chat: <Cpu size={32} className="text-muted-foreground/20" />,
    write: <PenLine size={32} className="text-blue-400/30" />,
    correct: <FileSearch size={32} className="text-amber-400/30" />,
    research: <Search size={32} className="text-purple-400/30" />,
    plan: <LayoutPanelLeft size={32} className="text-emerald-400/30" />,
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[300px] text-center px-6 py-8">
      <div className="mb-4">{icons[mode]}</div>
      <h2 className="text-[16px] font-semibold text-foreground mb-1">{titles[mode]}</h2>
      <p className="text-[12.5px] text-muted-foreground mb-6 max-w-[300px]">
        {mode === "correct"
          ? "Attach a file or paste your text, then send"
          : "Type, speak, or attach files to get started"}
      </p>
...
function InputFallback() {
  return (
    <div className="h-[88px] rounded-2xl border border-border bg-secondary/30 animate-pulse" />
  );
}

function modePlaceholder(mode: CzarMode): string {
  return {
    chat: "Ask CZAR anything… (voice or text)",
